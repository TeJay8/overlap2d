    ...
    private void computeBoundBox() {
        ...
        for (int i = 0, n = skeleton.getSlots().size; i < n; i++) {
            ...
            if ((attachment instanceof RegionAttachment)) {
                ...
                vertices = new float[region.getOffset().length];
                region.computeWorldVertices(slot.getBone(), vertices, 0, 0);
            }
            if ((attachment instanceof MeshAttachment)) {
                MeshAttachment region = (MeshAttachment) attachment;
                int size = region.getVertices().length;
                vertices = new float[size];
                region.computeWorldVertices(slot, 0, size, vertices, 0, 0);
            }
